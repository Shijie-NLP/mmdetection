...
from mmdet.utils import ConfigType, InstanceList, MultiConfig, OptConfigType, OptInstanceList
...
class AnchorFreeHead(BaseDenseHead):
    ...
    def __init__(
        ...
        loss_cls: ConfigType = dict(type="FocalLoss", use_sigmoid=True, gamma=2.0, alpha=0.25, loss_weight=1.0),
    ...
    def _init_cls_convs(self) -> None:
        ...
        for i in range(self.stacked_convs):
            ...
            conv_norm_bias_cfg = {"conv_cfg": conv_cfg, "norm_cfg": self.norm_cfg, "bias": self.conv_bias}
            self.cls_convs.append(ConvModule(chn, self.feat_channels, 3, stride=1, padding=1, **conv_norm_bias_cfg))
    ...
    def _init_reg_convs(self) -> None:
        ...
        for i in range(self.stacked_convs):
            ...
            if self.dcn_on_last_conv and i == self.stacked_convs - 1:
                ...
                conv_cfg = self.conv_cfg
            conv_norm_bias_cfg = {"conv_cfg": conv_cfg, "norm_cfg": self.norm_cfg, "bias": self.conv_bias}
            self.reg_convs.append(ConvModule(chn, self.feat_channels, 3, stride=1, padding=1, **conv_norm_bias_cfg))
    ...
    def aug_test(
        self, aug_batch_feats: list[Tensor], aug_batch_img_metas: list[list[Tensor]], rescale: bool = False
    ) -> list[ndarray]:
