...
class SinePositionalEncoding(BaseModule):
    ...
    def __init__(
        ...
        if normalize and not isinstance(scale, (float, int)):
            raise TypeError(
    ...
    def forward(self, mask: Tensor, input: Optional[Tensor] = None) -> Tensor:
        ...
        if mask is not None:
            B, H, W = mask.size()
            device = mask.device
            # For the convenience of exporting to ONNX,
            # it's required to convert `masks` from bool to int.
...
class LearnedPositionalEncoding(BaseModule):
    ...
    def __init__(
        ...
        init_cfg: MultiConfig = dict(type="Uniform", layer="Embedding"),  # noqa
    ) -> None:
        super().__init__(init_cfg=init_cfg)
...
class SinePositionalEncoding3D(SinePositionalEncoding):
    ...
    def forward(self, mask: Tensor) -> Tensor:
        ...
        """
        assert mask.dim() == 4, (
            f"{mask.shape} should be a 4-dimensional Tensor, got {mask.dim()}-dimensional Tensor instead "
        )
        # For the convenience of exporting to ONNX,
        # it's required to convert `masks` from bool to int.
        mask = mask.to(torch.int)
        ...
        dim_t_z = torch.arange((self.num_feats * 2), dtype=torch.float32, device=mask.device)
        ...
        # use `view` instead of `flatten` for dynamically exporting to ONNX
        B, T, H, W = mask.size()
        pos_x = torch.stack((pos_x[:, :, :, :, 0::2].sin(), pos_x[:, :, :, :, 1::2].cos()), dim=5).view(B, T, H, W, -1)
        pos_y = torch.stack((pos_y[:, :, :, :, 0::2].sin(), pos_y[:, :, :, :, 1::2].cos()), dim=5).view(B, T, H, W, -1)
        pos_z = torch.stack((pos_z[:, :, :, :, 0::2].sin(), pos_z[:, :, :, :, 1::2].cos()), dim=5).view(B, T, H, W, -1)
