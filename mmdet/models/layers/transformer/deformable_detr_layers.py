...
class DeformableDetrTransformerEncoder(DetrTransformerEncoder):
    ...
    @staticmethod
    def get_encoder_reference_points(spatial_shapes: Tensor, valid_ratios: Tensor, device: DeviceType) -> Tensor:
...
class DeformableDetrTransformerDecoder(DetrTransformerDecoder):
    ...
    def _init_layers(self) -> None:
        ...
        self.layers = ModuleList(
            [DeformableDetrTransformerDecoderLayer(**self.layer_cfg) for _ in range(self.num_layers)]
        )
        self.embed_dims = self.layers[0].embed_dims
