...
class DetrTransformerDecoder(BaseModule):
    ...
    def __init__(
        ...
        num_layers: int,
        layer_cfg: ConfigType,
        post_norm_cfg: OptConfigType = dict(type="LN"),  # noqa
...
class DetrTransformerEncoderLayer(BaseModule):
    ...
    def __init__(
        self,
        self_attn_cfg: OptConfigType = dict(embed_dims=256, num_heads=8, dropout=0.0),  # noqa
        ffn_cfg: OptConfigType = dict(  # noqa
            embed_dims=256, feedforward_channels=1024, num_fcs=2, ffn_drop=0.0, act_cfg=dict(type="ReLU", inplace=True)
        ),
        norm_cfg: OptConfigType = dict(type="LN"),  # noqa
        init_cfg: OptConfigType = None,
    ) -> None:
        super().__init__(init_cfg=init_cfg)
        ...
        if "batch_first" not in self.self_attn_cfg:
            ...
            assert self.self_attn_cfg["batch_first"] is True, (
                "First dimension of all DETRs in mmdet is `batch`, please set `batch_first` flag."
            )
...
class DetrTransformerDecoderLayer(BaseModule):
    ...
    def __init__(
        self,
        self_attn_cfg: OptConfigType = dict(embed_dims=256, num_heads=8, dropout=0.0, batch_first=True),  # noqa
        cross_attn_cfg: OptConfigType = dict(embed_dims=256, num_heads=8, dropout=0.0, batch_first=True),  # noqa
        ffn_cfg: OptConfigType = dict(  # noqa
            embed_dims=256,
