    ...
    def decode(
        ...
        if self.use_box_type:
            assert decoded_bboxes.size(-1) == 4, (
                "Cannot warp decoded boxes with box type when decoded boxes have shape of (N, num_classes * 4)"
            )
...
class DeltaXYWHBBoxCoderForGLIP(DeltaXYWHBBoxCoder):
    ...
    To completely match the official performance, we need to perform
    special calculations in the encoding and decoding processes, such as
    additional +1 and -1 calculations. However, this is not a user-friendly design.
    """
