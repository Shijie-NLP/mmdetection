...
def setup_cache_size_limit_of_dynamo():
    ...
    if digit_version(torch.__version__) >= digit_version("2.0.0"):
        if "DYNAMO_CACHE_SIZE_LIMIT" in os.environ:
            ...
            print_log(
                f"torch._dynamo.config.cache_size_limit is force set to {cache_size_limit}.",
                logger="current",
...
def register_all_modules(init_default_scope: bool = True) -> None:
    ...
    if init_default_scope:
        never_created = DefaultScope.get_current_instance() is None or not DefaultScope.check_instance_created("mmdet")
        if never_created:
