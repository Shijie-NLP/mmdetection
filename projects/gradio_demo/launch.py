...
class PanopticSegTab(ObjectDetectionTab):
    model_list = ["panoptic_fpn_r50_fpn_1x_coco", "mask2former_swin-s-p4-w7-224_8xb2-lsj-50e_coco-panoptic"]
...
class OpenVocabObjectDetectionTab:
    ...
    def create_ui(self):
        ...
        with gr.Row():
            example_images = gr.Dataset(
                components=[image_input, text_input], samples=[["demo/demo.jpg", "bench . car ."]]
            )
    ...
    def inference(self, model, image, text):
        ...
        results_dict = det_inferencer(
            image, texts=text, custom_entities=True, pred_score_thr=0.5, return_vis=True, no_save_vis=True
        )
        vis = results_dict["visualization"][0]
        return vis
...
class GroundingDetectionTab(OpenVocabObjectDetectionTab):
    ...
    def create_ui(self):
        ...
        with gr.Row():
            example_images = gr.Dataset(
                components=[image_input, text_input], samples=[["demo/demo.jpg", "There are a lot of cars here."]]
            )
            example_images.click(fn=self.update, inputs=example_images, outputs=[image_input, text_input])
    ...
    def inference(self, model, image, text):
        det_inferencer = DetInferencer(model, scope="mmdet", device=get_free_device())
        results_dict = det_inferencer(
            image, texts=text, custom_entities=False, pred_score_thr=0.5, return_vis=True, no_save_vis=True
        )
        vis = results_dict["visualization"][0]
        return vis
...
class OpenVocabInstanceSegTab(OpenVocabObjectDetectionTab):
    ...
    def inference(self, model, image, text):
        det_inferencer = DetInferencer(**self.model_info[model], scope="mmdet", device=get_free_device())
        results_dict = det_inferencer(image, texts=text, return_vis=True, no_save_vis=True)
        vis = results_dict["visualization"][0]
        return vis
...
class OpenVocabPanopticSegTab(OpenVocabObjectDetectionTab):
    ...
    def create_ui(self):
        ...
        with gr.Row():
            ...
            example_images.click(
                fn=self.update, inputs=example_images, outputs=[image_input, text_input, stuff_text_input]
            )
    ...
    def inference(self, model, image, text, stuff_text):
        det_inferencer = DetInferencer(**self.model_info[model], scope="mmdet", device=get_free_device())
        results_dict = det_inferencer(image, texts=text, stuff_texts=stuff_text, return_vis=True, no_save_vis=True)
        vis = results_dict["visualization"][0]
        return vis
...
class ImageCaptionTab:
    ...
    def inference(self, model, image):
        ...
        results_dict = ic_inferencer(image, return_vis=False, no_save_vis=True, return_datasamples=True)
...
class ReferImageCaptionTab(OpenVocabInstanceSegTab):
    ...
    def create_ui(self):
        ...
        with gr.Row():
            example_images = gr.Dataset(components=[image_input, text_input], samples=[["demo/demo.jpg", "tree"]])
            example_images.click(fn=self.update, inputs=example_images, outputs=[image_input, text_input])
    ...
    def inference(self, model, image, text):
        ...
        results_dict = ric_inferencer(image, texts=text, return_vis=True, no_save_vis=True)
        vis = results_dict["visualization"][0]
        return vis
...
class TextToImageRetrievalTab:
    ...
    def inference(self, model, prototype, text):
        ...
        results_dict = retri_inferencer(inputs, texts=text, return_vis=True, no_save_vis=True)
        vis = results_dict["visualization"][0]
        return vis
